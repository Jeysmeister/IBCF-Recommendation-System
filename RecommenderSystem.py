...
current_user = ""
user_id = ""
title_recently_rated = []

def get_current_rated(user_id_current, user_current):
    global gameTitle_list_of_current_user
...
def register():
    ...
    # email
    email_label = Label(register_screen, text="Email").pack(padx=40, side=TOP, anchor="w")
    email_entry = Entry(register_screen, textvariable=email, width=35)
    email_entry.pack()
...
def register_user():
    ...
    if username_info in users:
        ...
        if users == []:
            ...
            fa.write(str(len(users)) + "||" + username_info + "||" + password_info + "\n")
        fa.close()
...
def email_register():
    ...
    for user_list_line in user_list:
        ...
        fa = open('email.txt', "a")
        if users == []:
            fa.write(str(0) + "||" + username_info + "||" + email_info + "\n")
        else:
            fa.write(str(len(users) + 1) + "||" + username_info + "||" + email_info + "\n")
        fa.close()
...
def already_bought_checker():
    games_list = []
    user_list = []
    current_user_games_list = []
    fr = open('mygames.txt')
    my_games_list = fr.readlines()
    fr.close()
    for item in my_games_list:
        item_split = item.split('::')
        user_list.append(item_split[0])
        games_list.append(item_split[1])

    if current_user in user_list:
        user_index = user_list.index(current_user)
        current_user_games_list = games_list[user_index].split("||")
        current_user_games_list[len(current_user_games_list) - 1] = current_user_games_list[len(current_user_games_list) - 1][:-1]
    else:
        return False

    if selected_game in current_user_games_list:
        return True
    else:
        return False

def already_bought_message():
    messagebox.showinfo("Item already bought", "You've already bought this item.")

def buy():
    already_bought = already_bought_checker()
    if already_bought:
...
def succesfully_rated():
    messagebox.showinfo("Notice","Game Successfully Rated")
def already_rated():
    messagebox.showinfo("Notice","Game Already Rated")
...
def rategame():
    ...
    def exportdata():
        ...
        user_list_name = []
        value_rated = False
        if title in gameTitle_list_of_current_user:
            value_rated = True
        if title in title_recently_rated:
            value_rated = True
        if value_rated == False:
            succesfully_rated()
            title_recently_rated.append(title)
            for i in list(range(0, len(Data))):
                if value == Data[i][1]:
                    gameId = Data[i][0]
            f = open('users.txt', "r")
            ...
            if user in user_list_name:
                ...
                usersId = user_list_id[index_user]
            rating = get_current_value()
            epoch = int(time.time())
            data_list = [usersId, gameId, rating, epoch]
            print(data_list)
            with open('ratinggames.csv', 'a', newline='') as f_object:
